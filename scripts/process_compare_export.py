...
with open(IN_CSV, 'rb') as csv_file:
    ...
    for row in reader:
        ...
        if len(time[0]) == 3:
            time[0] = time[0][1:]
        timestamp = datetime.strptime(row['Date Start (Baseline)']+' '+str(1+int(time[0]))+':'+str(1+int(time[1])),'%m/%d/%Y %H:%M')
        ...
        # speed = float(row['Average Speed (Comparison)'])
        speed = float(row['Average Speed KPH (Baseline)'])
        if speed > max_speed:
            max_speed = speed
        ...
        # observations = int(float(row['Number of Observations (Comparison)']))
        observations = int(float(row['Number of Observations (Baseline)']))
        ...
        # Extract Observation number
        # deviation = float(row['Standard Deviation (Comparison)']))
        deviation = float(row['Standard Deviation (Baseline)'])
